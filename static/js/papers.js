...
const updateCards = (papers) => {
  ...
  all_mounted_cards.select(".card-title").on("click", function (d) {
    const iid = d.id;
    all_mounted_cards
      .filter((d) => d.id === iid) // eslint-disable-line no-shadow
...
const render = () => {
  ...
  Object.keys(filters).forEach((k) => {
    filters[k] ? f_test.push([k, filters[k]]) : null; // eslint-disable-line no-unused-expressions
  });
    ...
    const fList = allPapers.filter((d) => {
      ...
      while (i < f_test.length && pass_test) {
        if (f_test[i][0] === "titles") {
          pass_test &= // eslint-disable-line no-bitwise
